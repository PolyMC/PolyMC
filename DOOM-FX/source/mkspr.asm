...
;***************************************************************************
	...
	xref	SpriteList,SpriteListName
	xref	SpriteDef,SpriteDefPtr
...
ScanArgs
	...
	bne	ScanArgs			; Nope
	move.b	(a0)+,d0
	cmp.b	#'L',d0				; -L	SPRITELIST NAME?
	bne	ScanArgs100
	lea	SpriteListName,a0
SaveArg
	move.l	-4(a1),d0
	addq.l	#2,d0
	move.l	d0,(a0)
...
DoneArgs100
	...
	addq.w	#8,a0
	move.l	(a0),SpriteList
	addq.w	#8,a0
	move.l	(a0),SpriteDef
	move.l	(a0),SpriteDefPtr
;
	move.l	#MSGNoSpriteList,d7
	tst.l	SpriteListName
	beq	Error
	jsr	DoMakeSprite			; Make Sprite Data!
...
MemoryListStart
	dc.l	(MEMF_PUBLIC!MEMF_CLEAR)		; SPRITE LIST FILE
	dc.l	(16384)
	dc.l	(MEMF_PUBLIC!MEMF_CLEAR)		; DOOM PALETTE DATA
	dc.l	(256*3)
	dc.l	(MEMF_PUBLIC!MEMF_CLEAR)		; SPRITE BITMAP DATA
	dc.l	(32*1024)
MemoryListEnd
