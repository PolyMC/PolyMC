...
class MetaEntry {
   ...
   public:
    ...
    void setStale(bool stale) { this->m_stale = stale; }

    auto getFullPath() -> QString;

    auto getRemoteChangedTimestamp() -> QString { return m_remote_changed_timestamp; }
    void setRemoteChangedTimestamp(QString remote_changed_timestamp) { this->m_remote_changed_timestamp = remote_changed_timestamp; }
    void setLocalChangedTimestamp(qint64 timestamp) { m_local_changed_timestamp = timestamp; }

    auto getETag() -> QString { return m_etag; }
    void setETag(QString etag) { this->m_etag = etag; }

    auto getMD5Sum() -> QString { return m_md5sum; }
    void setMD5Sum(QString md5sum) { this->m_md5sum = md5sum; }
...
class HttpMetaCache : public QObject {
   ...
   public:
    ...
    // get the entry from cache and verify that it isn't m_stale (within reason)
    auto resolveEntry(QString base, QString resource_path, QString expected_etag = QString()) -> MetaEntryPtr;

    // add a previously resolved m_stale entry
   ...
   private:
    // create a new m_stale entry, given the parameters
